"""
GermEval 2026 – Subtask 2: DBO Classification  |  Sprint 2
============================================================
4-Modell-Ensemble: GBERT-large + DeBERTa-v3-base + mDeBERTa-v3-base + GElectra-large
Soft-Voting + Threshold-Tuning fuer Minderheitsklassen (agitation, subversive)

Aenderungen gegenueber Sprint 1 (germeval2026_ensemble_train.py):
  - XLM-RoBERTa entfernt (F1=0.31 auf Val, zog Ensemble runter)
  - mDeBERTa-v3-base und GElectra-large neu hinzugefuegt
  - --threshold_agitation / --threshold_subversive fuer Recall-Tuning

VERWENDUNG:
    python germeval2026_sprint2_train.py --augmentation paraphrase --out_dir output/sprint2
...
DATEI-LAYOUT (erwartet):
    ...
    synthethic_data/
        paraphrased.csv            <- LLM-Paraphrasen (text, label)  [optional]

AUSGABE:
    <out_dir>/
        model_gbert/               <- GBERT-large Gewichte (float16)
        model_deberta/             <- DeBERTa-v3-base Gewichte (float16)
        model_mdeberta/            <- mDeBERTa-v3-base Gewichte (float16)
        model_gelectra/            <- GElectra-large Gewichte (float16)
...
# 1. CLI
# ------------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="GermEval 2026 Sprint 2 Ensemble Training")
...
parser.add_argument("--paraphrase_file", default="synthethic_data/paraphrased.csv")
...
# Threshold-Tuning fuer Minderheitsklassen
parser.add_argument("--threshold_agitation",  type=float, default=0.5,
                    help="Min. Wahrscheinlichkeit um als 'agitation' zu klassifizieren (default: 0.5)")
parser.add_argument("--threshold_subversive", type=float, default=0.5,
                    help="Min. Wahrscheinlichkeit um als 'subversive' zu klassifizieren (default: 0.5)")
...
# Ausgabe
parser.add_argument("--out_dir",     default="models/sprint2")
...
# ------------------------------------------------------------------------------
# 2. Modell-Konfigurationen (4 Modelle, kein XLM-RoBERTa)
# ------------------------------------------------------------------------------
MODEL_CONFIGS = {
    "gbert": {
...
print(f"\n{'='*65}")
print(f"  GermEval 2026 - Sprint 2  |  4-Modell-Ensemble")
print(f"{'='*65}")
print(f"  Device:              {DEVICE}")
print(f"  Augmentierung:       {args.augmentation}")
print(f"  Threshold agitation: {args.threshold_agitation}")
print(f"  Threshold subversive:{args.threshold_subversive}")
...
# ------------------------------------------------------------------------------
# 5. Threshold-basierte Prediction
# ------------------------------------------------------------------------------
def apply_thresholds(probs, classes, thr_agitation, thr_subversive):
    """
    Ersetzt reines argmax: Minderheitsklassen gewinnen schon ab ihrem Threshold,
    auch wenn eine andere Klasse (meist 'nothing') hoehere Wahrscheinlichkeit hat.
    Reihenfolge: agitation wird vor subversive geprueft.
    """
    thresholds = {"agitation": thr_agitation, "subversive": thr_subversive}
    classes_list = list(classes)
    preds = []
    for p in probs:
        pred = None
        for cls, thr in thresholds.items():
            if cls in classes_list and p[classes_list.index(cls)] >= thr:
                pred = cls
                break
        preds.append(pred if pred else classes[p.argmax()])
...
# ------------------------------------------------------------------------------
# 7. Modell-Architektur
# ------------------------------------------------------------------------------
class TransformerClassifier(nn.Module):
    """MLP-Head auf [CLS]-Token."""
...
# ------------------------------------------------------------------------------
# 8. Training eines einzelnen Modells
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 9. Externer Checkpoint (optional)
# ------------------------------------------------------------------------------
def load_external_checkpoint(ckpt_path, base_model_id, shortname):
    ...
    report       = classification_report(all_labels, all_preds,
                                         target_names=CLASSES, zero_division=0)
    print(f"\n  [{shortname}] Finaler Report (nur Inferenz):")
    print(report)
...
# ------------------------------------------------------------------------------
# 11. Ensemble Soft-Voting auf Val-Set (mit Threshold-Tuning)
# ------------------------------------------------------------------------------
print(f"\n{'─'*65}")
...
ensemble_preds = apply_thresholds(
    ensemble_probs, le.classes_,
    args.threshold_agitation, args.threshold_subversive,
)
ensemble_preds_idx = le.transform(ensemble_preds)

ensemble_macro_f1  = f1_score(val_labels_arr, ensemble_preds_idx, average="macro",   zero_division=0)
ensemble_per_class = f1_score(val_labels_arr, ensemble_preds_idx, average=None,
                               zero_division=0, labels=list(range(N_CLASSES)))
ensemble_report    = classification_report(val_labels_arr, ensemble_preds_idx,
                                           target_names=CLASSES, zero_division=0)

print(f"\n  Ensemble Macro-F1 (Val): {ensemble_macro_f1:.5f}")
print(f"\n  Einzelmodelle zum Vergleich:")
for short, res in model_results.items():
    ext_tag = " [extern]" if res.get("is_external") else ""
    print(f"    {short:<12} Macro-F1: {res['best_val_macro_f1']:.5f}{ext_tag}")
print(f"\n  Ensemble Classification Report:")
print(ensemble_report)

# ------------------------------------------------------------------------------
# 12. Testset-Vorhersagen (mit Threshold-Tuning)
# ------------------------------------------------------------------------------
print(f"\n{'─'*65}")
...
if len(df_test) > 0:
    ...
    if USE_FOURTH:
        print(f"  Lade {args.fourth_model_name} (extern) fuer Testvorhersage ...")
    ...
    test_preds_labels   = apply_thresholds(
        test_ensemble_probs, le.classes_,
        args.threshold_agitation, args.threshold_subversive,
    )
...
# ------------------------------------------------------------------------------
# 14. Globale results.csv anhaengen
# ------------------------------------------------------------------------------
results_row = {
    "timestamp":           datetime.now().strftime("%Y-%m-%d %H:%M"),
    "script":              "germeval2026_sprint2_train.py",
    "model_type":          f"ensemble_{n_models}model_sprint2",
    "models":              "+".join(cfg["model_id"] for cfg in MODEL_CONFIGS.values()),
    "augmentation":        args.augmentation,
    "aug_files":           "|".join(aug_files_used) if aug_files_used else "none",
    "threshold_agitation": args.threshold_agitation,
    "threshold_subversive":args.threshold_subversive,
...
report_text = f"""GermEval 2026 - Sprint 2: 4-Modell-Ensemble Training Report
{sep}
Timestamp:            {datetime.now().strftime('%Y-%m-%d %H:%M')}
Augmentierung:        {args.augmentation}
Modelle:              {n_models} (GBERT + DeBERTa + mDeBERTa + GElectra)
Threshold agitation:  {args.threshold_agitation}
Threshold subversive: {args.threshold_subversive}
...
print(f"\n{'='*65}")
print(f"  SPRINT 2 ABGESCHLOSSEN")
print(f"{'='*65}")
